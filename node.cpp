...
namespace dir_tree {
    std::string node_process_ctx::get_str() const {
        ...
        if (this->metadata_year != "") {
            result += " year:" + this->metadata_year;
        }
        if (this->hash != "") {
    ...
    node::node(std::filesystem::path const &p, bool skip_probe) : path{p}, filetype{std::filesystem::file_type::none},
                                                                  exists{false} {
        if (!skip_probe)
            this->probe();
    }
